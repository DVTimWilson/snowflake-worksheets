...
WITH logins(login_date, logoff_date, close_date) AS (
    SELECT '2022-03-13' :: DATE, '2022-03-20' :: DATE, '2022-03-21' :: DATE
    UNION ALL
    ...
    UNION ALL
    SELECT '2022-03-21' :: DATE, NULL :: DATE, '2022-03-31' :: DATE
    UNION ALL
    SELECT NULL :: DATE, NULL :: DATE, '2022-03-31' :: DATE
    UNION ALL
    SELECT '2022-03-18' :: DATE, '2022-03-21' :: DATE, NULL :: DATE
),
...
SELECT logins.login_date
    ,logins.logoff_date
    ,logins.close_date

    ,(SELECT COUNT(*)
        FROM calendar_dim
        WHERE dim_date BETWEEN logins.login_date AND logins.logoff_date
            AND working_day_flag = 1
    ) AS session_wd_calc
    ,(SELECT COUNT(*)
        FROM calendar_dim
        WHERE dim_date BETWEEN logins.login_date AND logins.close_date
            AND working_day_flag = 1
    ) AS total_wd_calc
FROM logins
ORDER BY logins.login_date ASC
;
